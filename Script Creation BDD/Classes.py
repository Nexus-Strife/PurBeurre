...
class NewDB:  # Class permettant de créer la bdd
    ...
    def create_tb(cursor):
        ...
        try:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS usr_products(name VARCHAR(255), store VARCHAR(255),"
                           "grade VARCHAR(1), cat VARCHAR(50), prod_id INT, foreign key (prod_id) REFERENCES products(prod_id), cat_id INT, foreign key (cat_id) REFERENCES categories(id))")
