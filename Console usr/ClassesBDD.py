...
class Console:  # Main class of the program
    ...
    def selcat(self, cursor):  # Method where the user can select a category to explore
        ...
        while sel_cat:
            ...
            try:  # A try ... except block that check if the user enter the id of a category rather than the name
                ...
                cursor.execute("SELECT id FROM categories WHERE id = %s", (id_sel, ))
                # This SQL req check if the id of the category entered before exist
                    ...
                    while another_prime_menu:
                        ...
                        try:  # Another try...except block that will check if 'product_id' only contain int
                            ...
                            while ctrl_nbr_products:
                                ...
                                # This cursor.execute check if the product_id exist in the database

                                id_exist = cursor.fetchone()

                                if not id_exist:
                                    print("\nVotre produit n'est pas dans la base de données")
                                    # If not exist, notify it to the user
                                ...
                                try:  # Another one here to check the value of the next var ( 'go_again' )
                                    ...
                                    if go_again == 1:
                                        ...
                                        ''' The three next cursor.execute put in var the category ID, 
                                            the grade and the energy per 100g that going to compose the search args
                                        ...
                                        while add_sub:
                                            try:
                                                prod_sub = int(input("Veuillez entrer l'ID du substitut"
                                                                     " que vous souhaitez: "))
    ...
    def find(self, cursor):  # Method that give a search fonction
        ...
        while find_it:
            ...
            # The user enter the name of the product that he is searching
            aliment = (input("\nVeuillez entrer le nom d'un aliment que vous recherchez:"))

            # SQL req that check every products by name that user entered, using operator "LIKE"
            cursor.execute("SELECT * FROM products"
                           " WHERE name LIKE %(p)s ORDER by grade ASC", {"p": "%{}%".format(aliment.replace(' ', '%'))})
    ...
    def find_substitute(self, cursor):  # Method that find a substitute for an ID given as product
        ...
        while find_substitute_while:
            ...
            try:
                # The user need to enter the product's ID of the one he want to substitute
                ...
                for row in cursor:
                    ...
                    for row in cursor:
                        ...
                        # ...Here in case of the user forgot for what he asking a substitute ({prod_id) {name})
    ...
    def user_products(self, cursor):  # Method that describe how the user_products's database work
        ...
        ''' This cursor.execute grab every informations about products and their substitutes. since the products id and
            the substitute id exist in the same table, the program need to create alias from tables and rows to display
            everything'''
        ...
        for row in cursor:            # "\033[4m" display the word with underlines
            ...
            # If there is not a substitute id for a given id product then don't print anything
            if ("{sub_prod_id}".format(**row)) != "None":
